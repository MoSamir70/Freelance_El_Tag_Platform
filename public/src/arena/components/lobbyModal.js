// src/arena/components/lobbyModals.js
// نافذة اللوبي المنبثقة للاعبين (تعرض قائمة اللاعبين، زر جاهز، مغادرة)
// ✅ تم تحسينها لضمان:
// - الاستماع الحي لتحديثات الغرفة
// - تحديث زر الجاهزية وحالة اللاعبين
// - التحويل التلقائي إلى شاشة السباق عند بدء السباق (status === 'playing')
// - إغلاق المودال بشكل صحيح قبل التوجيه
...
export async function showLobbyModal(roomId) {
    ...
    document.getElementById('leave-lobby-btn')?.addEventListener('click', async () => {
        ...
        // تحديث الصفحة لإعادة تحميل حالة المستخدم
    ...
    if (readyBtn) {
        readyBtn.addEventListener('click', async () => {
            // تبديل حالة الجاهزية: إذا كان جاهزاً يصبح غير جاهز والعكس
            const room = await getRoomSnapshot(currentRoomId);
            const myPlayer = room?.players?.find(p => p.id === currentUser.id);
            const newReadyState = !(myPlayer?.isReady || false);
            await setPlayerReady(currentRoomId, newReadyState);
        });
    }
...
async function getRoomSnapshot(roomId) {
    // استدعاء getDocumentOnce من firestoreSync
    const { getDocumentOnce } = await import('../../online/core/firestoreSync.js');
...
async function startRoomListening() {
    ...
    roomUnsubscribe = subscribeToDocument('activeRooms', currentRoomId, (room) => {
        ...
        updateRoomUI(room);
        // ✅ التحقق من بدء السباق: إذا كانت الغرفة في حالة 'playing' ولديها sessionId
        if (room.status === 'playing' && room.sessionId) {
            // إغلاق المودال أولاً ثم التوجيه إلى شاشة السباق
            closeLobbyModal();
            showToast('🏁 بدأ السباق! جاري تحويلك...', 'success');
            // الانتقال إلى واجهة السباق مع تمرير sessionId
